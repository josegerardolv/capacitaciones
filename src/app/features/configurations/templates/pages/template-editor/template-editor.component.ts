...
import { Subscription, switchMap, concat, of, tap, last, forkJoin } from "rxjs";
...
import {
  SelectComponent,
  SelectOption,
} from "../../../../../shared/components/inputs/select.component";
  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  @ViewChild("canvasScrollContainer", { static: false })
  canvasScrollRef!: ElementRef<HTMLDivElement>;

  // Document / Template state
  documentId: number | null = null;
  templateDocument: TemplateDocument | null = null;
  template: CertificateTemplate | null = null;
  /** Variables originales del design (para persistencia); las dinámicas se cargan siempre desde API */
  private designVariables: TemplateVariable[] = [];
  /** Grupos asociados al template con sus enrollments (para obtener variables dinámicas) */
  groupsWithEnrollments: GroupWithEnrollments[] = [];

  // Fabric.js canvas
  private canvas!: fabric.Canvas;
  private isCanvasInitialized = false;
  /** Último textbox seleccionado (para insertar variables al hacer clic en el panel, que pierde la selección) */
  private lastSelectedTextbox: fabric.Textbox | null = null;
  ...
  readonly standardPageSizes: Record<
    string,
  ...
  readonly fontFamilyOptions: SelectOption[] = this.fontFamilies.map((f) => ({
    value: f,
    label: f,
  }));
  readonly fontSizeOptions: SelectOption[] = this.fontSizes.map((s) => ({
    value: s,
    label: `${s}px`,
  }));
  ...
  ngOnInit(): void {
    ...
    if (this.documentId) {
      ...
      this.loadDynamicVariablesForNewTemplate();
    }
  }
  ...
  handleKeyboardEvent(event: KeyboardEvent): void {
    ...
    if (
      ...
      if (activeObject) {
        ...
        switch (event.key) {
          case "ArrowUp":
            activeObject.top! -= nudge;
            break;
          case "ArrowDown":
            activeObject.top! += nudge;
            break;
          case "ArrowLeft":
            activeObject.left! -= nudge;
            break;
          case "ArrowRight":
            activeObject.left! += nudge;
            break;
    ...
    this.canvas.on("selection:created", (e: any) => {
      this.ngZone.run(() => {
        const obj = (e.selected?.[0] ??
          e.target ??
          this.canvas.getActiveObject()) as CustomFabricObject;
        if (obj?.type === "textbox")
          this.lastSelectedTextbox = obj as fabric.Textbox;
  ...
  /** Carga variables dinámicas para plantilla nueva (sin documento guardado) */
  private loadDynamicVariablesForNewTemplate(): void {
    this.requirementsService.getRequirements().subscribe({
      next: (requirements) => {
        const dynamicVars =
          TemplateService.mapRequirementsToVariables(requirements);
        const editorVars = TemplateService.buildEditorVariables(
          [],
          dynamicVars,
        );
      ...
      error: (err) => {
        console.error("Error loading requirements for new template:", err);
        this.template = {
          id: 0,
          name: "Nuevo Template",
          description: "",
  ...
  loadDocument(id: number): void {
    forkJoin({
      doc: this.templateService.getTemplateById(id),
      requirements: this.requirementsService.getRequirements(),
      groups: this.templateService.getGroupsForTemplate(id),
    }).subscribe({
      next: ({ doc, requirements, groups }) => {
        this.templateDocument = doc;
        const design = TemplateService.parseDesign(doc.fields);
        const designVars = design?.variables || [];
        this.designVariables = designVars;

        if (groups.length > 0) {
          this.groupsWithEnrollments = groups;
          const groupVars = this.getAvailableVariablesFromGroups();
          const mergedVars = TemplateService.buildEditorVariables(
            designVars,
            groupVars,
          );
          this.template = {
      ...
      error: (err) => {
        console.error("Error loading document, requirements or groups:", err);
      },
    });
  }
  ...
  private createTextObject(element: CanvasElement): CustomFabricObject {
    ...
    const textbox = new fabric.Textbox(config?.content || "Texto", {
      ...
      fontStyle:
        (config?.fontStyle as "" | "italic" | "normal" | "oblique") || "normal",
  ...
  private createShapeObject(element: CanvasElement): CustomFabricObject | null {
    ...
    if (config?.strokeDashArray && config.strokeDashArray !== "solid") {
      ...
      shape.set(
        "strokeDashArray",
        dashPatterns[config.strokeDashArray] || null,
      );
  ...
  private createImageObject(element: CanvasElement): Promise<void> {
    ...
    if (!src || isDynamic) {
      ...
      return fabric.FabricImage.fromURL(placeholderDataUrl).then((img) => {
        ...
        customImg.imageFit =
          element.imageConfig?.fit === "cover" ||
          element.imageConfig?.fit === "fill"
            ? element.imageConfig.fit
            : "contain";
        if (element.visualStyle?.opacity != null) {
  ...
  private createQRObject(element: CanvasElement): Promise<void> {
    ...
    const qrSize =
      ...
      Math.min(element.transform.width, element.transform.height, 150) ||
      150;
  ...
  updateBackgroundFit(fit: "cover" | "contain" | "fill"): void {
    ...
    if (this.canvas.backgroundImage) {
      this.applyBackgroundFit(
        this.canvas.backgroundImage as fabric.FabricImage,
        fit,
      );
    }
    if (this.template) {
   ...
   */
  private createImageElementFromDataUrl(
    dataUrl: string,
    opts?: {
      counterKey?: string;
      namePrefix?: string;
    ...
    fabric.FabricImage.fromURL(dataUrl).then((img) => {
      ...
      // Name and counter
      this.elementCounters[counterKey] =
        (this.elementCounters[counterKey] || 0) + 1;
  ...
  deleteSelected(): void {
    const activeObject = this.canvas.getActiveObject();
    if (activeObject) {
      if (this.lastSelectedTextbox === activeObject)
  ...
  duplicateSelected(): void {
    ...
    activeObject.clone().then((cloned: fabric.FabricObject) => {
      ...
      customCloned.elementType = (
        activeObject as CustomFabricObject
      ).elementType;
      customCloned.elementName = (
        activeObject as CustomFabricObject
      ).elementName;
  ...
  filterVariables(): void {
    ...
    if (this.selectedVariableCategory !== "all") {
      filtered = filtered.filter(
        (v) => v.category === this.selectedVariableCategory,
      );
    }
  ...
  /** Obtiene el textbox destino para insertar variables (activo o último seleccionado) */
  private getTargetTextboxForVariable(): fabric.Textbox | null {
    const active = this.canvas?.getActiveObject();
    if (active?.type === "textbox") return active as fabric.Textbox;
    if (
      this.lastSelectedTextbox &&
      this.canvas?.getObjects().includes(this.lastSelectedTextbox)
    ) {
      return this.lastSelectedTextbox;
    }
    return null;
  }
  ...
  canInsertVariable(variable: TemplateVariable): boolean {
    ...
    const targetTextbox = this.getTargetTextboxForVariable();
  ...
  insertVariable(variable: TemplateVariable): void {
    ...
    if (
      ...
      const textbox = this.getTargetTextboxForVariable();
      if (!textbox) return;
      ...
      if (currentText === "Texto de ejemplo" || currentText.trim() === "") {
        textbox.set("text", varTag);
      } else {
        textbox.set("text", currentText + " " + varTag);
      }

      (textbox as CustomFabricObject).isDynamic = true;
      // No sobreescribir variableName: el contenido usa formato {{variable}}
      // y el PDF sustituye todas las ocurrencias via substituteVariables()
      textbox.dirty = true;
      textbox.initDimensions();
      textbox.setCoords();
  ...
  private updatePropertiesFromObject(obj: CustomFabricObject): void {
    ...
    if (!(obj as any).isMarginGuide) {
      displayX = Math.max(
        bounds.left,
        Math.min(bounds.right - objWidth, displayX),
      );
      displayY = Math.max(
        bounds.top,
        Math.min(bounds.bottom - objHeight, displayY),
      );
    }
    ...
    if (
      ...
      values.stroke =
        (obj.stroke as string) || (obj as any).stroke || "#6B7280";
    ...
    if (obj.elementType === "qr") {
      values.qrContent = obj.variableName
        ? `{{${obj.variableName}}}`
        : "https://example.com";
  ...
  onPropertyChange(property: string, value: any): void {
    ...
    switch (property) {
      case "x": {
        const numX = this.parseNumericValue(
          value,
          undefined,
          undefined,
          activeObject.left ?? 0,
        ...
        const clampedX = Math.max(
          bounds.left,
          Math.min(bounds.right - objWidth, numX),
        );
      ...
      case "y": {
        const numY = this.parseNumericValue(
          value,
          undefined,
          undefined,
          activeObject.top ?? 0,
        ...
        const clampedY = Math.max(
          boundsY.top,
          Math.min(boundsY.bottom - objHeight, numY),
        );
      ...
      case "width": {
        const numWidth = this.parseNumericValue(
          value,
          1,
          undefined,
        ...
        activeObject.scaleX =
          (activeObject.scaleX || 1) * (numWidth / currentWidth);
        activeObject.setCoords();
        this.enforceMarginBoundaries(activeObject as CustomFabricObject);
        break;
      }
      case "height": {
        const numHeight = this.parseNumericValue(
          value,
          1,
          undefined,
        ...
        activeObject.scaleY =
          (activeObject.scaleY || 1) * (numHeight / currentHeight);
        activeObject.setCoords();
        this.enforceMarginBoundaries(activeObject as CustomFabricObject);
        break;
      }
      case "rotation": {
        const numRotation = this.parseNumericValue(
          value,
          -360,
          360,
      ...
      case "fontSize":
        if (activeObject.type === "textbox") {
          const textbox = activeObject as fabric.Textbox;
          const numFontSize = this.parseNumericValue(
            value,
            1,
            500,
            textbox.fontSize ?? 16,
      ...
      case "lineHeight":
        if (activeObject.type === "textbox") {
          const textbox = activeObject as fabric.Textbox;
          const numLineHeight = this.parseNumericValue(
            value,
            0.5,
            5,
      ...
      case "charSpacing":
        if (activeObject.type === "textbox") {
          const textbox = activeObject as fabric.Textbox;
          const numCharSpacing = this.parseNumericValue(
            value,
            -50,
            500,
            textbox.charSpacing ?? 0,
          );
      ...
      case "stroke":
        ...
        break;
      case "strokeWidth": {
        const numStrokeWidth = this.parseNumericValue(
          value,
          0,
          100,
      ...
      case "borderRadius":
        if (activeObject.type === "rect") {
          ...
          const numBorderRadius = this.parseNumericValue(
            value,
            0,
            500,
            rect.rx ?? 0,
      ...
      case "qrSize": {
        ...
        const currentQrSize =
          qrObj.qrSize ?? (Math.round(activeObject.getScaledWidth()) || 150);
        ...
        if (oldObj.elementType === "image") {
          if (value) {
            ...
            fabric.FabricImage.fromURL(dataUrl).then((img) => {
              ...
              this.elementPropertiesForm.patchValue(
                { isDynamic: true, variableName: "", imageSrc: "" },
                { emitEvent: false },
              );
              this.canvas.renderAll();
              this.saveHistory();
            ...
            this.elementPropertiesForm.patchValue(
              {
                ...
                name: variable
                  ? variable.label
                  : this.elementPropertiesForm.get("name")?.value,
                imageSrc: "",
              },
              { emitEvent: false },
            );
  ...
  undo(): void {
    if (this.historyIndex > 0) {
      ...
      this.historyIndex--;
      this.canvas
        .loadFromJSON(JSON.parse(this.history[this.historyIndex]))
        .then(() => {
          this.canvas.renderAll();
  ...
  startSidebarResize(side: "left" | "right", event: PointerEvent): void {
    ...
    this.sidebarStartWidth =
      side === "left" ? this.leftPanelWidth : this.rightPanelWidth;
  ...
  saveTemplate(): void {
    ...
    const design: CanvasDesign = {
      pageConfig: {
        ...(this.template?.pageConfig ||
          this.templateService.getDefaultPageConfig()),
    ...
    const saveWithUploads = (templateId: number) => {
      const uploads: {
        type: "image" | "qr";
        file: File;
        replaceFn: (url: string) => void;
      }[] = [];

      if (design.pageConfig.backgroundImage?.startsWith("data:")) {
        const file = TemplateService.base64ToFile(
          design.pageConfig.backgroundImage,
          "background.png",
        );
        uploads.push({
          type: "image",
          file,
          replaceFn: (url) => (design.pageConfig.backgroundImage = url),
        });
      }
      ...
      for (const el of design.elements) {
        if (el.imageConfig?.src?.startsWith("data:")) {
          ...
          uploads.push({
            type: "image",
            file,
            replaceFn: (url) => (el.imageConfig!.src = url),
          });
        }
      }
      ...
      const sendFields = () => {
        ...
        this.templateService
          .configureTemplate(templateId, fields)
          .subscribe(onSaveResult);
      };
      ...
      if (uploads.length > 0) {
        concat(
          ...uploads.map((u) =>
            this.templateService
              .uploadMedia(templateId, u.file, u.type)
              .pipe(tap((url) => u.replaceFn(url))),
          ),
        )
          .pipe(last())
          .subscribe({
  ...
  private getShapeType(
    fabricType: string,
  ): "rectangle" | "circle" | "ellipse" | "triangle" | "line" | "polygon" {
    const map: Record<
      string,
  ...
  isShapeSelected(): boolean {
    ...
    return (
      [
        ...
        "polygon",
        "group",
      ].includes(type || "") || elementType === "shape"
    );
  }

  isImageSelected(): boolean {
    return (
      this.selectedObject?.elementType === "image" ||
  ...
  onReplaceImage(event: Event): void {
    ...
    reader.onload = (e) => {
      ...
      if (
        activeObject &&
        (activeObject as CustomFabricObject).elementType === "image"
      ) {
        ...
        fabric.FabricImage.fromURL(dataUrl).then((img) => {
          ...
          this.elementPropertiesForm.patchValue(
            { imageSrc: dataUrl, isDynamic: false, variableName: "" },
            { emitEvent: false },
          );
          this.saveHistory();
        });
  ...
  regenerateQR(): void {
    ...
    const qrContent =
      this.elementPropertiesForm.get("qrContent")?.value ||
      "https://example.com";
    const qrSize = this.elementPropertiesForm.get("qrSize")?.value || 150;
    const isDynamic =
      this.elementPropertiesForm.get("isDynamic")?.value || false;
    const variableName =
      this.elementPropertiesForm.get("variableName")?.value || "";

    // Generate new QR code
    const displayContent =
      isDynamic && variableName ? `{{${variableName}}}` : qrContent;
  ...
  drawMarginGuides(): void {
    ...
    const zoom =
      this.canvas && this.canvas.getZoom ? this.canvas.getZoom() || 1 : 1;
    const canvasActualWidth =
      this.canvas && this.canvas.getWidth
        ? (this.canvas.getWidth() as number)
        : this.canvasWidth;
    const canvasActualHeight =
      this.canvas && this.canvas.getHeight
        ? (this.canvas.getHeight() as number)
        : this.canvasHeight;
    ...
    const bottomLine = new fabric.Line(
      [
        0,
        canvasActualHeight - marginBottom * zoom,
        canvasActualWidth,
        canvasActualHeight - marginBottom * zoom,
      ],
      { ...guideStyle },
    ...
    const rightLine = new fabric.Line(
      [
        canvasActualWidth - marginRight * zoom,
        0,
        canvasActualWidth - marginRight * zoom,
        canvasActualHeight,
      ],
  ...
  async previewPDF(): Promise<void> {
    ...
    try {
      ...
      const previewVariables = this.pdfGenerator.getPreviewVariables(
  ...
  private showAlignmentGuides(movingObj: CustomFabricObject): void {
    if (
      !movingObj ||
      (movingObj as any).isMarginGuide ||
      (movingObj as any).isGridLine
    ) {
      return;
    }
    ...
    objects.forEach((other) => {
      ...
      if (Math.abs(objCenterX - otherCenterX) < this.snapThreshold) {
        ...
        this.addVerticalGuide(otherCenterX, {
          ...guideStyle,
          strokeDashArray: [5, 5],
        });
      }
      ...
      if (Math.abs(objCenterY - otherCenterY) < this.snapThreshold) {
        ...
        this.addHorizontalGuide(otherCenterY, {
          ...guideStyle,
          strokeDashArray: [5, 5],
        });
      }
  ...
  distributeHorizontally(): void {
    ...
    const lastRight =
      bounds.right - objects[objects.length - 1].getScaledWidth();
  ...
  distributeVertically(): void {
    ...
    const lastBottom =
      bounds.bottom - objects[objects.length - 1].getScaledHeight();

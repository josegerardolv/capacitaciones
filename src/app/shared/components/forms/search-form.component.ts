        ...
        <form [formGroup]="searchForm" class="p-8 space-y-6">
          ...
          <div *ngIf="showMainSearch" class="relative">
            ...
            <input type="text" 
                   ...
                   class="w-full pl-12 pr-12 py-4 border-2 border-gray-300 rounded-lg focus:ring-4 focus:ring-institucional-guinda focus:ring-opacity-20 focus:border-institucional-guinda transition-all duration-200 form-input-transition text-lg placeholder-gray-400"
          ...
          <div *ngIf="searchFields.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div *ngFor="let field of searchFields" class="space-y-2 animate-form-step-in">
              <label class="block text-sm font-semibold text-gray-700">
                <span *ngIf="field.icon" class="material-icons text-sm mr-2 text-institucional-guinda">{{ field.icon }}</span>
                {{ field.label }}
              </label>
              ...
              <input *ngIf="field.type === 'text'" 
                     ...
                     class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-institucional-guinda focus:ring-opacity-50 focus:border-institucional-guinda transition-all duration-200 form-input-transition">
              ...
              <select *ngIf="field.type === 'select'" 
                      [formControlName]="field.key"
                      class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-institucional-guinda focus:ring-opacity-50 focus:border-institucional-guinda transition-all duration-200 form-input-transition bg-white">
              ...
              <label *ngIf="field.type === 'boolean'" class="flex items-center gap-3 cursor-pointer p-2 rounded-lg hover:bg-gray-50 transition-colors">
                <input type="checkbox" 
                       [formControlName]="field.key"
                       class="w-5 h-5 text-institucional-guinda border-2 border-gray-300 rounded focus:ring-institucional-guinda focus:ring-2 transition-all">
          ...
          <div *ngIf="quickFilters.length > 0" class="border-t border-gray-200 pt-6">
            <h4 class="text-sm font-semibold text-gray-700 mb-4 flex items-center gap-2">
              <span class="material-icons text-lg text-institucional-guinda">tune</span>
              Filtros rápidos
            </h4>
            <div class="flex flex-wrap gap-3">
              <button *ngFor="let filter of quickFilters" 
                      ...
                      [class.border-institucional-guinda]="isQuickFilterActive(filter.key)"
                      [class.bg-institucional-guinda]="isQuickFilterActive(filter.key)"
                      ...
                      [class.hover:border-institucional-guinda]="!isQuickFilterActive(filter.key)"
          ...
          <div class="flex items-center justify-between pt-6 border-t border-gray-200">
            <div class="flex items-center gap-4">
              ...
              <div *ngIf="resultCount !== undefined" class="flex items-center gap-2 px-3 py-2 bg-gray-100 rounded-lg">
                <span class="material-icons text-lg text-institucional-guinda">analytics</span>
            ...
            <div class="flex items-center gap-3">
              ...
              <button *ngIf="allowSaveSearch" 
                      type="button"
                      class="inline-flex items-center gap-2 px-4 py-2 text-sm border-2 border-institucional-vino text-institucional-vino hover:bg-institucional-vino hover:text-white rounded-lg transition-all duration-200 form-button-transition"
                      [disabled]="activeFiltersCount === 0"
                      [class.opacity-50]="activeFiltersCount === 0"
                      [class.cursor-not-allowed]="activeFiltersCount === 0"
